def upload_artifacts(sqa=false, commit_sha="null", run_number="null") {
    ...
    {
        def output = sh(script: "python3 -u third_party/matter_github_jenkins/artifacts/upload_artifacts.py --branch_name ${env.BRANCH_NAME} --sqa ${sqa} --commit_sha ${commit_sha} --run_number ${run_number}", returnStdout: true).trim()
...
def send_test_results_to_github(commit_sha, sqa_tests_result, sqa_tests_summary) {
    ...
    {
        sh """
            python3 -u third_party/matter_github_jenkins/github/send_results_to_github.py --commit_sha ${commit_sha} --sqa_result ${sqa_tests_result} --sqa_summary "${sqa_tests_summary}" --jenkins_url ${env.BUILD_URL}
        """
    }
}
