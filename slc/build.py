...
def build_with_arg(board: str, components: str | None) -> list[str]:
    """Build component arguments for --with flag, similar to build.sh build_with_arg function."""
    if components:
        return ["--with", f"{board},{components}"]
    else:
        return ["--with", board]


def build_without_arg(components: str | None) -> list[str]:
    """Build component arguments for --without flag, similar to build.sh build_without_arg function."""
    if components:
...
Optional Arguments:
  ...
  --ci                  CI mode (continuous integration build)
  --with <options>      Append additional options to the board using comma separation
  --with_app <components>    Specify additional components for application build (solutions only)
  --without_app <components> Exclude specific components from application build (solutions only)
  --with_bootloader <components>    Specify additional components for bootloader build (solutions only)
  --without_bootloader <components> Exclude specific components from bootloader build (solutions only)
  --cmake               Generate project using cmake instead of makefile
...
Examples:
  ...
  ./build.py solution.slcw brd4187c --with_app component1,component2
  ./build.py solution.slcw brd4187c --without_bootloader component3,component4
...
Solution-Specific Component Management:
  The --with_app, --without_app, --with_bootloader, and --without_bootloader options
  allow fine-grained control over which components are included in .slcw solution builds:
  - --with_app: Add specific components to the application build
  - --without_app: Remove specific components from the application build
  - --with_bootloader: Add specific components to the bootloader build
  - --without_bootloader: Remove specific components from the bootloader build
  
  NOTE: These options are only used for .slcw solution files and are silently 
  ignored for .slcp project files.
...
def classify_extra_args(raw_extras):
    ...
    without_app_components(str|None), with_bootloader_components(str|None), 
    ...
    with_app_components = None
    without_app_components = None
    with_bootloader_components = None
    ...
    while i < len(raw_extras):
        ...
        if token == "--skip_gen":
            ...
            if i + 1 >= len(raw_extras):
                sys.exit("--with_app requires a value")
            with_app_components = raw_extras[i + 1]
            i += 2
        elif token.startswith("--with_app "):
            with_app_components = token[len("--with_app "):]
            i += 1
        elif token == "--without_app":
            if i + 1 >= len(raw_extras):
                sys.exit("--without_app requires a value")
            without_app_components = raw_extras[i + 1]
            i += 2
        elif token.startswith("--without_app "):
            without_app_components = token[len("--without_app "):]
            i += 1
        elif token == "--with_bootloader":
            if i + 1 >= len(raw_extras):
                sys.exit("--with_bootloader requires a value")
            with_bootloader_components = raw_extras[i + 1]
            i += 2
        elif token.startswith("--with_bootloader "):
            with_bootloader_components = token[len("--with_bootloader "):]
            i += 1
        elif token == "--without_bootloader":
            if i + 1 >= len(raw_extras):
                sys.exit("--without_bootloader requires a value")
            without_bootloader_components = raw_extras[i + 1]
            i += 2
        elif token.startswith("--without_bootloader "):
    ...
    return dict(skip_gen=skip_gen, sisdk=sisdk, output_suffix=output_suffix,
                ...
                with_app_components=with_app_components, without_app_components=without_app_components,
                with_bootloader_components=with_bootloader_components, without_bootloader_components=without_bootloader_components,
