...
* Copy
```
cp -r $SDK_DIR/protocol/openthread/sample-apps/ot-cli/trustzone .
cp $SDK_DIR/protocol/openthread/sample-apps/ot-tz-secure-app/ot-tz-secure-application.slcp trustzone/
```

* Combined
```
rm -Rf out/trustzone; slc generate -daemon -w slc/apps/lighting-app/thread/trustzone/lighting-app.slcw --with brd4187c --sdk third_party/simplicity_sdk -d out/trustzone
make -j8 -C out/trustzone/ -f lighting-app.solution.Makefile
kf out/trustzone/artifact/lighting-app-full.s37
```

* Script
```
rm -Rf out/brd4187c/lighting-app-solution; ./slc/build.sh slc/apps/lighting-app/thread/trustzone/lighting-app.slcw brd4187c
```
