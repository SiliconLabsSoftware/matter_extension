...
`CommonAppTask` extends the base AppTask through the Curiously Recurring Template Pattern (CRTP). You override only the `*Impl()` methods you need, the base declares one `*Impl()` per overridable API. Steps:

1. Find the method to override in the base API (see [Override API reference](#override-api-reference) below).
2. Declare the same method signature in `CommonAppTask` in your `CommonAppTask.h` under `private:`
...
`AppTask.cpp` under `autogen/` (see [CommonAppTask](#commonapptask) above). That
...
- **Methods that already exist in the autogenerated layer** — Customize them by
  overriding the matching `*Impl()` method in `CommonAppTask`. Do not edit the
...
The following shows a minimal example `CommonAppTask` that overrides `AppInitImpl()` (required) and `ButtonEventHandlerImpl()`.
