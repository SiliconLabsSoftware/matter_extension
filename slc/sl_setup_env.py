...
class MatterEnvSetup:
    ...
    def set_root_paths(self):
        ...
        self.tools_folder_path = os.path.join(
            self.silabs_chip_root, "slc", "tools")
    ...
    def set_tool_urls_and_paths(self):
        ...
        if p == "win32":
            ...
            self.commander_path = os.path.join(
                t, "SimplicityCommander-Windows")
            ...
            self.arm_gcc_download_dir = os.path.join(
                t, "arm-gnu-toolchain-12.2.rel1-mingw-w64-i686-arm-none-eabi")
            ...
            self.commander_app_path = os.path.join(
                self.commander_path, "Commander.app", "Contents", "MacOS")
            self.java_path = os.path.join(
                r, "slc", "tools", "amazon-corretto-21.jdk")
            self.arm_gcc_dir = os.path.join(
                t, "arm-gnu-toolchain-12.2.rel1-darwin-arm64-arm-none-eabi")
            ...
            self.java_path = os.path.join(
                r, "slc", "tools", "amazon-corretto-21.0.7.6.1-linux-x64")
            self.commander_app_path = os.path.join(
                self.commander_path, "commander")
            self.arm_gcc_dir = os.path.join(
                t, "arm-gnu-toolchain-12.2.rel1-x86_64-arm-none-eabi")
            self.ninja_path = os.path.join(t, "ninja", "ninja")
    ...
    def download_and_extract_arm_toolchain(self):
        if not os.path.isfile(os.path.join(self.arm_toolchain_path, "arm-none-eabi-gcc")) and not os.path.isfile(os.path.join(self.arm_toolchain_path, "arm-none-eabi-gcc.exe")):
            ...
            if self.platform == "win32":
                ...
                os.rename(self.arm_gcc_download_dir, os.path.join(
                    self.tools_folder_path, "gcc"))
            else:
                dload.save(self.arm_toolchain_url, os.path.join(
                    self.tools_folder_path, "gcc.tar.xz"))
                tar = tarfile.open(os.path.join(
                    self.tools_folder_path, "gcc.tar.xz"), "r:xz")
    ...
    def download_and_extract_java(self):
        if not os.path.isdir(self.java_path):
            ...
            if self.platform == "win32":
                dload.save(self.java_url, os.path.join(
                    self.tools_folder_path, "java.zip"))
                ...
                dload.save(self.java_url, os.path.join(
                    self.tools_folder_path, "java.tar.gz"))
                tar = tarfile.open(os.path.join(
                    self.tools_folder_path, "java.tar.gz"), "r:gz")
                tar.extractall(path=self.tools_folder_path)
                tar.close()
    ...
    def download_and_extract_slc_cli(self):
        if not os.path.isfile(os.path.join(self.slc_cli_path, "slc")):
            ...
            dload.save_unzip(
                slc_cli_url, self.tools_folder_path, delete_after=True)
            st = os.stat(os.path.join(
                self.tools_folder_path, "slc_cli", "slc"))
            os.chmod(os.path.join(self.tools_folder_path,
                     "slc_cli", "slc"), st.st_mode | stat.S_IEXEC)
            if self.platform == "darwin":
                ...
                st = os.stat(os.path.join(self.tools_folder_path,
                             "slc_cli", "bin", "slc-cli", "slc-cli"))
                os.chmod(os.path.join(self.tools_folder_path, "slc_cli",
                         "bin", "slc-cli", "slc-cli"), st.st_mode | stat.S_IEXEC)
        else:
    ...
    def download_and_extract_zap(self, install_without_checking=False):
        ...
        if (not os.path.isfile(os.path.join(self.zap_path, "zap-cli")) and not os.path.isfile(os.path.join(self.zap_path, "zap.exe"))) or install_without_checking:
            ...
            if self.platform == "darwin":
                ...
                os.chmod(os.path.join(self.zap_path, "zap-cli"),
                         st.st_mode | stat.S_IEXEC)
                zap_root = os.path.join(
                    self.zap_path, "zap.app", "Contents", "MacOS", "zap")
                st = os.stat(zap_root)
                os.chmod(zap_root, st.st_mode | stat.S_IEXEC)
            elif self.platform == "linux":
                dload.save_unzip(zap_url, os.path.join(
                    self.tools_folder_path, "zap"), delete_after=True)
                st = os.stat(os.path.join(
                    self.tools_folder_path, "zap", "zap-cli"))
                os.chmod(os.path.join(self.tools_folder_path, "zap",
                         "zap-cli"), st.st_mode | stat.S_IEXEC)
                st = os.stat(os.path.join(
                    self.tools_folder_path, "zap", "zap"))
                os.chmod(os.path.join(self.tools_folder_path,
                         "zap", "zap"), st.st_mode | stat.S_IEXEC)
    ...
    def check_and_update_zap_version(self):
        ...
        installed_zap_version = re.search(
            r"Version:\s*([\d.]+)", output).group(1)
        min_zap = self.MINIMUM_ZAP_REQUIRED.replace(
            "v", "").replace("-nightly", "")
        date_format = "%Y.%m.%d"
        installed_zap_version = datetime.strptime(
            installed_zap_version, date_format)
        ...
        if installed_zap_version < min_zap:
            ...
            try:
                logging.info(
                    f"Deleting old version of zap located at {self.zap_path}")
    ...
    def download_and_extract_commander(self):
        ...
        if not (os.path.isfile(os.path.join(self.commander_path, "Commander.app", "Contents", "MacOS", "commander")) or
            ...
            dload.save_unzip(
                commander_url, self.tools_folder_path, delete_after=True)
            commander_zip = None
    ...
    def download_and_extract_ninja(self):
        ...
        try:
            if not os.path.isfile(self.ninja_path):
                ...
                dload.save_unzip(ninja_url, os.path.join(
                    self.tools_folder_path, "ninja"), delete_after=True)
            else:
    ...
    def create_extension_symlinks(self):
        ...
        logging.info(
            "Creating symbolic links for matter_extension and wiseconnect")
    ...
    def write_env_file(self):
        if self.platform == "darwin":
            ...
            with open(env_path, "w") as outfile:
                ...
                outfile.write(
                    f"POST_BUILD_EXE={os.path.join(self.commander_app_path, 'commander')}\n")
                outfile.write(f"ARM_GCC_DIR={self.arm_gcc_dir}\n")
                outfile.write(
                    f"JAVA17_HOME={os.path.join(self.java_path, 'Contents', 'Home')}\n")
            ...
            env_path = os.path.expanduser(
                os.path.join(self.tools_folder_path, ".env"))
            with open(env_path, "w") as outfile:
                outfile.write(f'STUDIO_ADAPTER_PACK_PATH={self.zap_path}\n')
                outfile.write(
                    f'POST_BUILD_EXE={os.path.join(self.commander_path, "Simplicity Commander", "commander.exe")}\n')
                outfile.write(f'ARM_GCC_DIR={self.arm_gcc_dir}\n')
                outfile.write(f'JAVA17_HOME={self.java_path}\n')
                outfile.write('ZAP_INSTALL_PATH=' +
                              self.zap_path.replace("\\", "/")+'\n')
                outfile.write(
                    f'TOOLS_PATH={self.slc_cli_path};{self.zap_path};{self.arm_toolchain_path};{self.java_path}\\bin;\n')
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup environment for Matter project using Silicon Labs Configurator.")
