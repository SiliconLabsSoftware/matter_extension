...
def resolve_matter_version(cli_version: Optional[str]) -> str:
    ...
    try:
        ...
        # Nothing found: log error and exit (typo in filename intentionally corrected here in message)
        logging.error("Unable to determine Matter package version: provide --matter-version or create 'matter_package_version' file with a version string.")
        sys.exit(1)
...
def generate_pkg_slt_files(base_directory, verbose, common, matter_version):
    ...
    for root, dirs, files in os.walk(base_directory):
        ...
        if common:
            # Force use of the universal content for every project
            pkg_slt_content = pkg_slt_content_all
        else:
            # Use path components for safer detection (avoids partial substring matches)
            ...
            if "siwx917" in path_components:
                ...
                # Fallback: just the common content
