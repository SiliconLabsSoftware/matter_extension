...
def main():
    ...
    for board, current in sorted(current_data.items()):
        if board in baseline_data:
            ...
            flash_exceeds = flash_pct > FLASH_THRESHOLD_PCT
            ram_exceeds = ram_pct > RAM_THRESHOLD_PCT
            ...
            # Format flash change
            flash_sign = "+" if flash_diff >= 0 else ""
            flash_str = f"{flash_sign}{flash_diff:,} bytes ({flash_sign}{flash_pct:.2f}%)"
            
            # Format RAM change
            ram_sign = "+" if ram_diff >= 0 else ""
            ram_str = f"{ram_sign}{ram_diff:,} bytes ({ram_sign}{ram_pct:.2f}%)"
