...
def replace_text(filename, regex_to_search, replacement_text, warning_if_unchanged=True):
    with open(filename, "r") as rfile:
        ...
        if (replaced_count == 0 and warning_if_unchanged):
            print("Warning: Following regex format not found in " +
                  filename+". Review manually to ensure format has not changed.")
...
if __name__ == '__main__':
    ...
    #   - id: matter
    #     version: "0.0.3"
    slcp_files = [os.path.abspath(f)
                  for f in pathlib.Path(ROOT).glob("slc/**/*.slcp")]
    for file in slcp_files:
        replace_text(file, "sdk_extension:\n *- id: matter\n *version: \""+VERSION_REGEX_FORMAT +
    ...
    replace_text(str(ROOT)+"/matter.slce", "id: matter\nversion: " +
                 VERSION_REGEX_FORMAT, "id: matter\nversion: "+EXTENSION_NEW_VERSION)
    replace_text(str(ROOT)+"/matter.slce", "version: " +
                 VERSION_REGEX_FORMAT+"", "version: "+EXTENSION_NEW_VERSION)
    ...
    replace_text(str(ROOT)+"/matter.slce", "  *id: simplicity_sdk\n  *version: " +
                 VERSION_REGEX_FORMAT, "  id: simplicity_sdk\n  version: "+GDSK_NEW_VERSION)
    ...
    if FULL_VERSION:
        replace_text(str(ROOT)+"/matter.slsdk", FULL_VERSION_REGEX,
                     EXTENSION_NEW_VERSION+"-"+AUX_VERSION)
    replace_text(str(ROOT)+"/matter.slsdk", "version=" +
                 VERSION_REGEX_FORMAT, "version="+EXTENSION_NEW_VERSION)
    ...
    replace_text(str(ROOT)+"/third_party/matter_private/jenkins/pipeline_metadata.yml",
                 "matterExtensionVersion: '"+VERSION_REGEX_FORMAT+"'", "matterExtensionVersion: '"+EXTENSION_NEW_VERSION+"'")
    ...
    if (UPDATE_README):
        readme_files = [
            os.path.abspath(f) for f in pathlib.Path(ROOT).glob("slc/**/*.md")]
        for file in readme_files:

            replace_text(file, "https://docs.silabs.com/matter/"+VERSION_REGEX_FORMAT,
                         "https://docs.silabs.com/matter/"+EXTENSION_NEW_VERSION)
            if FULL_VERSION:
                replace_text(file, "https://www.silabs.com/documents/public/software/SilabsMatterPi_"+FULL_VERSION_REGEX+"-extension.zip",
                             "https://www.silabs.com/documents/public/software/SilabsMatterPi_"+EXTENSION_NEW_VERSION+"-"+AUX_VERSION+"-extension.zip", False)
    ...
    if FULL_VERSION:
        replace_text(str(ROOT)+"/matter_docs.xml", "https://github.com/SiliconLabs/matter_extension/releases/tag/v" +
                     VERSION_REGEX_FORMAT+"", "https://github.com/SiliconLabs/matter_extension/releases/tag/v"+EXTENSION_NEW_VERSION)
        replace_text(str(ROOT)+"/matter_docs.xml", "Silicon Labs Matter v"+FULL_VERSION_REGEX +
                     "", "Silicon Labs Matter v"+EXTENSION_NEW_VERSION+"-"+AUX_VERSION+"")
