...
def build_with_sisdk_slc(sdk_path):
    ...
    slcp_file_path = os.path.join(
        sisdk_path, "protocol/openthread/sample-apps/ot-ncp/ot-rcp.slcp")
    ...
    subprocess.run(["slc", "signature", "trust",
                   "--sdk", sisdk_path], check=True)
    for board_name in boards:
        silabs_board = str(board_name).lower()
        output_path = os.path.join(
            root_path, "out/ot-rcp-binaries", silabs_board)
        try:
            subprocess.run(["slc", "generate", slcp_file_path, "-d",
                           output_path, "--with", silabs_board], check=True)
            subprocess.run(["make", "-f", "ot-rcp.Makefile",
                           "all"], cwd=output_path, check=True)
            shutil.copyfile("{}/build/debug/ot-rcp.s37".format(output_path),
                            "{}/ot-rcp/ot-rcp_{}.s37".format(root_path, silabs_board))
...
def main():
    parser = argparse.ArgumentParser(
        description='Build ot-rcp binaries from sisdk')
