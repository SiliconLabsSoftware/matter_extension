...
if __name__ == "__main__":
    ...
    for compile_commands_file_path in sys.argv[1:]:
        ...
        if os.path.isfile(os.path.join(root, compile_commands_file_path)):
            ...
            if MATTER_APP == "":
                print(
                    "Could Not found a matching app for {app}".format(
            ...
            for cmd in compile_commands:  # Loop through compile_commands.json
                ...
                file = (
                    dir / cmd["file"]
                ).resolve()  # concatenates the 'directory' and 'file' fields and resolves the resulting path which makes the path absolute, without any symlinks .. etc
                ...
                if (
                    "chip-efr32-" in lib or "chip-siwx917-" in lib
                ) and "example" in lib:
                    continue
                ...
                if (
                    "src" in file.parts
                    and "app" in file.parts
                    and "clusters" in file.parts
                ):
                    continue
                ...
                inc = map(  # creates a map object
                    ...
                    lambda x: str(
                        (dir / x.replace("-I", "", 1).replace("-isystem", "", 1))
                        .resolve()
                ...
                libs[lib]["defines"] |= set(
                    defines
                )  # update 'defines' entry of the dict

            os.makedirs(
                "slc/component/matter-core-sdk", exist_ok=True
            ...
            for name, data in libs.items():
                ...
                if name in [
                    s.lower().replace("-", "_") for s in APP_NAME_COMMON
                ...
                if (
                    name.startswith((APP_NAME_SILABS))
                    or name.startswith(("matter_silabs"))
                    or name.startswith(("provision"))
                ):
                    continue

                # Create a separate wifi component for these components
                if (
                    name in ["efr32", "dnssd", "libinetlayer", "libdevicelayer"]
                    and wifi == True
                ):
                    name += "_wifi"
                ...
                with open(
                    f"slc/component/matter-core-sdk/{name}.slcc", "w"
                ) as f:  # creates an SLCC file and opens in write mode
                    component = {}  # creates a dict called component
                    component["id"] = (
                        name  # creates an "id" entry and sets its value to 'name'
                    )
                    component["description"] = (
                        name  # does the same thing as the line above
                    )
                    ...
                    for src in sorted(
                        ...
                        if "silabs/third_party/connectedhomeip" in src:
                            src = src.partition("silabs/third_party/connectedhomeip")[
                                -1
                            ].lstrip("/")
                    ...
                    for inc in sorted(
                        ...
                        if any(
                            ...
                            for path in [
                                "openthread",
                                compile_commands_file_path.replace(
                                ...
                                "RTT",
                                "zzz_generated/" + MATTER_APP,
                                "examples/" + MATTER_APP + "/silabs/efr32/include",
                            ]
                        ):  # For Python-3.8
                            continue
                        if any(
                            ...
                            for path in [
                                "openthread",
                                sys.argv[1].removesuffix("compile_commands.json")
                                + "gen/include",
                                sys.argv[1].removesuffix("compile_commands.json")
                                + "protocol_buffer",
                                "RTT",
                                "zzz_generated/" + MATTER_APP,
                                "examples/" + MATTER_APP + "/silabs/include",
                            ]
                        ):
                            continue
                        if any(
                            ...
                            for path in [
                                ...
                                "RTT",
                                "zzz_generated/" + MATTER_APP,
                                "examples/" + MATTER_APP + "/silabs/SiWx917/include",
                            ]
                        ):
                            continue
                        # Skip any references to third_party/silabs
                        ...
                        if "silabs/third_party/connectedhomeip" in inc:
                            inc = inc.partition("silabs/third_party/connectedhomeip")[
                                -1
                            ].lstrip("/")
                        ...
                        if (
                            "/platform/silabs/efr32" in inc
                            or "/platform/silabs/SiWx917" in inc
                        ) and not ("efr32" in name or "siwx917" in name):
                            continue

                        component["include"].append(
                            {"path": inc}
                    ...
                    for define in sorted(
                        ...
                        if (
                            "=" in define
                        ):  # looks for defines and splits the string to a define, value pair
                        ...
                        if (
                            value is not None
                        ):  # if the value is not empty string, create a dict with the following keys and values and assigns it to the 'define' entry in the component dict
                    ...
                    # inserts the data in the component dict to the open SLCC file passed as 'f' in YAML format                    # inserts the data in the component dict to the open SLCC file passed as 'f' in YAML format
