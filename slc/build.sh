...
if [ "$skip_gen" = false ]; then
	if [[ "$SILABS_APP_PATH" == *.slcw ]]; then
		if [[ "$SILABS_APP_PATH" != *-siwx* ]] && [ "$GENERATE_BOOTLOADER" = true ]; then
			...
			run_slc_generate_with_retry generate -d "$OUTPUT_DIR" --sdk-package-path "$SISDK_ROOT" --sdk-package-path "$EXTENSION_DIR" --sdk-package-path "$WISECONNECT3_DIR" $PROJECT_FLAG "$SILABS_APP_PATH" $BOOTLOADER_WITH_ARG $BOOTLOADER_WITHOUT_ARG -pids bootloader $CONFIG_ARGS --generator-timeout=180 -o "$BUILD_SYSTEM"
			if [ $? -ne 0 ]; then
		...
		if [ "$GENERATE_TZ_SECURE" = true ]; then
			...
			run_slc_generate_with_retry generate -d "$OUTPUT_DIR" --sdk-package-path "$SISDK_ROOT" --sdk-package-path "$EXTENSION_DIR" --sdk-package-path "$WISECONNECT3_DIR" $PROJECT_FLAG "$SILABS_APP_PATH" $WITH_ARG -pids trustzone-secure $CONFIG_ARGS --generator-timeout=180 -o "$BUILD_SYSTEM"
			if [ $? -ne 0 ]; then
		...
		if [[ "$SILABS_APP_PATH" != *bootloader* ]] && [ "$GENERATE_APPLICATION" = true ]; then
			...
			run_slc_generate_with_retry generate -d "$OUTPUT_DIR" --sdk-package-path "$SISDK_ROOT" --sdk-package-path "$EXTENSION_DIR" --sdk-package-path "$WISECONNECT3_DIR" $PROJECT_FLAG "$SILABS_APP_PATH" $APP_WITH_ARG $APP_WITHOUT_ARG -pids application $CONFIG_ARGS --generator-timeout=180 -o "$BUILD_SYSTEM"
			if [ $? -ne 0 ]; then
				echo "ERROR: Failed to generate application for: $SILABS_APP_PATH"
				exit 1
			fi
		...
		run_slc_generate_with_retry generate -d "$OUTPUT_DIR" --sdk-package-path "$SISDK_ROOT" --sdk-package-path "$EXTENSION_DIR" --sdk-package-path "$WISECONNECT3_DIR" $PROJECT_FLAG "$SILABS_APP_PATH" --with "$SILABS_BOARD" $CONFIG_ARGS --generator-timeout=180 -o "$BUILD_SYSTEM"
		if [ $? -ne 0 ]; then
...
if [ "$GENERATE_BOOTLOADER" = true ] && [ "$GENERATE_APPLICATION" = false ]; then
	...
	if [ "$BUILD_SYSTEM" = "cmake" ]; then
		BOOTLOADER_CMAKE_DIR=$(find "$OUTPUT_DIR/matter-bootloader" -maxdepth 2 -type d -name "*_cmake" | head -1)
		if [ -z "$BOOTLOADER_CMAKE_DIR" ]; then
			echo "ERROR: No cmake directory found in $OUTPUT_DIR/matter-bootloader"
			exit 1
		fi
	...
	if [ "$BUILD_SYSTEM" = "cmake" ]; then
		APP_CMAKE_DIR=$(find "$OUTPUT_DIR" -mindepth 2 -maxdepth 3 -type d -name "*_cmake" ! -path "*/matter-bootloader/*" | head -1)
		if [ -z "$APP_CMAKE_DIR" ]; then
			echo "ERROR: No cmake directory found in $OUTPUT_DIR"
			exit 1
		fi
	...
	if [ "$BUILD_SYSTEM" = "cmake" ]; then
		if [[ "$SILABS_APP_PATH" == *.slcp ]]; then
			...
			while IFS= read -r cmake_dir; do
				echo "Building cmake project: $cmake_dir"
				if ! (cd "$cmake_dir" && cmake --workflow --preset project); then
					echo "ERROR: cmake build failed in $cmake_dir"
					exit 1
				fi
			done < <(find "$OUTPUT_DIR" -mindepth 2 -maxdepth 3 -type d -name "*_cmake")
		fi
	else
