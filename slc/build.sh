...
if [ "$skip_gen" = false ]; then
	if [[ "$SILABS_APP_PATH" == *.slcw ]]; then
		if [[ "$SILABS_APP_PATH" != *-siwx* ]] && [ "$GENERATE_BOOTLOADER" = true ]; then
			...
			run_slc_generate_with_retry generate -d $OUTPUT_DIR --sdk-package-path $SISDK_ROOT --sdk-package-path $EXTENSION_DIR --sdk-package-path $WISECONNECT3_DIR $PROJECT_FLAG $SILABS_APP_PATH $BOOTLOADER_WITH_ARG $BOOTLOADER_WITHOUT_ARG -pids bootloader $CONFIG_ARGS --generator-timeout=180
			if [ $? -ne 0 ]; then
		...
		if [ "$GENERATE_APPLICATION" = true ]; then
			...
			run_slc_generate_with_retry generate -d $OUTPUT_DIR --sdk-package-path $SISDK_ROOT --sdk-package-path $EXTENSION_DIR --sdk-package-path $WISECONNECT3_DIR $PROJECT_FLAG $SILABS_APP_PATH $APP_WITH_ARG $APP_WITHOUT_ARG -pids application $CONFIG_ARGS --generator-timeout=180
			if [ $? -ne 0 ]; then
