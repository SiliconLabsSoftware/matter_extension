...
[ -d $SISDK_ROOT/extension ] && echo "Directory $SISDK_ROOT/extension exists." || mkdir $SISDK_ROOT/extension
...
<<<<<<< HEAD
			run_slc_generate_with_retry generate --tt -s $GSDK_ROOT --daemon -d $OUTPUT_DIR $PROJECT_FLAG $SILABS_APP_PATH $BOOTLOADER_WITH_ARG $BOOTLOADER_WITHOUT_ARG -pids bootloader $CONFIG_ARGS --generator-timeout=180
=======
			run_slc_generate_with_retry generate -d $OUTPUT_DIR --sdk-package-path $SISDK_ROOT --sdk-package-path $EXTENSION_DIR --sdk-package-path $WISECONNECT3_DIR $PROJECT_FLAG $SILABS_APP_PATH $BOOTLOADER_WITH_ARG $BOOTLOADER_WITHOUT_ARG -pids bootloader $CONFIG_ARGS --generator-timeout=3500
>>>>>>> 5bc9c561e (Integrate with 25Q4)
			if [ $? -ne 0 ]; then
		...
		if [ "$GENERATE_APPLICATION" = true ]; then
			...
			run_slc_generate_with_retry generate --tt -s $GSDK_ROOT --daemon -d $OUTPUT_DIR $PROJECT_FLAG $SILABS_APP_PATH $APP_WITH_ARG $APP_WITHOUT_ARG -pids application $CONFIG_ARGS --generator-timeout=180
			if [ $? -ne 0 ]; then
