...
if [ "$skip_gen" = false ]; then
	if [[ "$SILABS_APP_PATH" == *.slcw ]]; then
		...
		run_slc_generate_with_retry generate -d $OUTPUT_DIR --sdk-package-path $SISDK_ROOT --sdk-package-path $EXTENSION_DIR --sdk-package-path $WISECONNECT3_DIR $PROJECT_FLAG $SILABS_APP_PATH $APP_WITH_ARG $APP_WITHOUT_ARG -pids application $CONFIG_ARGS --generator-timeout=3500
		if [ $? -ne 0 ]; then
		...
		run_slc_generate_with_retry generate -d $OUTPUT_DIR --sdk-package-path $SISDK_ROOT --sdk-package-path $EXTENSION_DIR --sdk-package-path $WISECONNECT3_DIR $PROJECT_FLAG $SILABS_APP_PATH --with $SILABS_BOARD $CONFIG_ARGS --generator-timeout=3500 -o makefile
		if [ $? -ne 0 ]; then
