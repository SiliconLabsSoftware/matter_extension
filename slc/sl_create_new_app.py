...
class createApp:
    ...
    def extract_and_save_paths(self):
        ...
        matter_sdk_src, matter_sdk_inc, project_name = self.extract_from_slcp(
            self.app_slcp_full_path)

        if self.use_solutions:
            self.sample_app_out_path = os.path.join(
                self.new_app_name, project_name)
        else:
        ...
        try:
            ...
            dest_dir = os.path.join(
                self.sample_app_out_path, '.bak', 'original', 'src')
            if os.path.exists(src_dir):
                shutil.copytree(src_dir, dest_dir, dirs_exist_ok=True)
                logging.info(f"Copied {src_dir} to {dest_dir}")
            else:
                logging.warning(
                    f"Source directory {src_dir} does not exist, skipping copy.")
        except Exception as e:
        ...
        try:
            ...
            dest_include_dir = os.path.join(
                self.sample_app_out_path, '.bak', 'original', 'include')
            if os.path.exists(include_dir):
                ...
                logging.warning(
                    f"Include directory {include_dir} does not exist, skipping copy.")
        except Exception as e:
            logging.error(f"Failed to copy include directory: {e}")
        trimmed_src_paths = [self.trim_to_str(
            p, 'third_party') for p in matter_sdk_src]
        trimmed_inc_paths = [self.trim_to_str(
            p, 'third_party') for p in matter_sdk_inc]
        customer_src = [os.path.join(
            self.sample_app_out_path, 'src', os.path.basename(p)) for p in trimmed_src_paths]
        customer_inc = [os.path.join(
            self.sample_app_out_path, 'include', os.path.basename(p)) for p in trimmed_inc_paths]
        upgrade_src_dir = os.path.join(
            self.sample_app_out_path, '.bak', 'original', 'src')
        upgrade_inc_dir = os.path.join(
            self.sample_app_out_path, '.bak', 'original', 'include')
        upgrade_src = []
        upgrade_inc = []
        ...
        output_file_json = os.path.join(
            self.sample_app_out_path, '.bak', 'source_and_include_paths.json')
        ...
        if not self.use_solutions:
            ...
            try:
                ...
                if app_slcp_path is None:
                    logging.error(
                        "No application project with .slcp path found in .slcw file.")
                    return

                self.app_slcp_full_path = os.path.join(
                    "slc", self.trim_to_str(app_slcp_path, "sample-app"))
                if not os.path.exists(self.app_slcp_full_path):
                    logging.error(
                        f"Application .slcp file does not exist: {self.app_slcp_full_path}")

                return self.app_slcp_full_path
            except Exception as e:
                logging.error(
                    f"Failed to parse .slcw file for application .slcp: {e}")

    def validate_board_argument(self, board):
        import re
        if not re.fullmatch(r"brd\d{4}[abc]", board, re.IGNORECASE):
            logging.error(
                "The second positional argument (silabs_board) must be of the form 'brdXXXXY', where XXXX are digits and Y is a/b/c.")
            return False
        return True
    ...
    def validate_arguments(self):
        ...
        if not os.path.exists(self.reference_project_file):
            logging.error(
                f"ReferenceProject File does not exist: {self.reference_project_file}")
            sys.exit(1)

        if not self.reference_project_file.endswith('.slcp') and not self.reference_project_file.endswith('.slcw'):
            logging.error(
                "ReferenceSlcp File should have a .slcp or .slcw extension")
            sys.exit(1)
        ...
        if self.silabs_board in self.SoC_boards:
            if not os.path.isdir(self.wiseconnect_root):
                logging.error(
                    f"The Wifi Extension required to build the {self.reference_project_file} does not exist at location:{self.wiseconnect_root}")
                sys.exit(1)
        ...
        if "sparkfun" in self.reference_project_file:
            third_party_hw_drivers_extension_path = os.path.join(
                os.getcwd(), "third_party", "third_party_hw_drivers_extension")
            subprocess.run(["git", "submodule", "update", "--init",
                           "--checkout", third_party_hw_drivers_extension_path])
            subprocess.run([self.slc_path, "signature", "trust",
                           "--extension-path", third_party_hw_drivers_extension_path])
    ...
    def get_environment(self):
        try:
            ...
            logging.error(
                "Could not load the .env file. Run sl_setup_env.py to generate .env file")
            sys.exit(1)
        ...
        if sys.platform == "win32":
            try:
                ...
                logging.error(
                    "SLC undefined. Set SLC from the sl_env_vars.bat")
    ...
    def trust_location(self, trust_locations):
        ...
        # Verify the location of sdk and extensions
            if not os.path.isdir(location):
                logging.error(
                    f"The {sdk_or_ext} does not exist at location:{location}")
                sys.exit(1)
            ...
            try:
                if sdk_or_ext == "SDK":
                    subprocess.run(
                        [self.slc_path, "configuration", "--sdk", location])
                    subprocess.run([self.slc_path, "signature",
                                   "trust", "--sdk", location])
                else:  # sdk_ext="Extension":
                    subprocess.run([self.slc_path, "signature",
                                   "trust", "--extension-path", location])
            except Exception as e:
                logging.error(
                    f"Error while trusting the location {location}: {e}")
                sys.exit(1)

    def slc_trust(self):
        # Get user permission to trust simplicity_sdk and extensions with use arm gcc
        # Use arm-gcc toolchain with slc
        subprocess.run([self.slc_path, "configuration",
                       "-gcc", self.arm_toolchain_path])
    ...
    def generate(self):
        if self.skip_gen:
            logging.info(
                "Skipping project generation as --skip-gen was specified.")
            self.extract_and_save_paths()
            return
        # Use appropriate build flag for sample-app/solutions
        project_flag = "-p" if self.reference_project_file.endswith(
            '.slcp') else "-w"
        ...
        try:
            cmd = [self.slc_path, "--java-location", self.java_path, "generate", "-d", self.new_app_name, project_flag, self.reference_project_file,
                   "--with", self.silabs_board+config_args, "--new-project", "--force", "--generator-timeout=180", "-o", "makefile"]
    ...
    def extract_and_save_paths(self):
        ...
        try:
            ...
            if os.path.exists(include_dir):
                ...
                print(
                    f"ERROR: Include directory {include_dir} does not exist.")
                sys.exit(1)
        except Exception as e:
...
def main():
    ...
    parser.add_argument("-n", "--new_app_name", dest="new_app_name",
                        required=False, help="Name of the new app directory to create")
    parser.add_argument("-p", "--reference_project_file", dest="reference_project_file",
                        required=False, help="Path to the reference .slcp or .slcw project file")
    parser.add_argument("-b", "--silabs_board", dest="silabs_board",
                        required=False, help="Silabs board name")
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Enable verbose (debug) logging")
    parser.add_argument("-s", "--skip_gen", action="store_true",
                        help="Skip project generation step")
    ...
    if not args.new_app_name or not args.reference_project_file or not args.silabs_board:
        if len(args.args) >= 3:
            ...
            parser.error(
                "new_app_name, reference_project_file, and silabs_board are required.")
    ...
    sys.argv = [sys.argv[0], args.new_app_name,
                args.reference_project_file, args.silabs_board]
