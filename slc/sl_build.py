...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Build a Matter application from a reference project file.")
    ...
    if not args.reference_project_file or not args.silabs_board:
        if len(args.args) >= 2:
            ...
            parser.error(
                "reference_project_file and silabs_board are required.")
    ...
    if reference_project_file.endswith('.slcp'):
        ...
        logging.info(
            "Example usage: python slc/sl_build.py <PathToReferenceProjectFile(.slcp or .slcw)> <SilabsBoard>")
        sys.exit(1)
    ...
    if not shutil.which("arm-none-eabi-gcc"):
        ...
        logging.error(
            "ERROR: please run slc/sl_setup_env.py to install arm gcc toolchain")
        sys.exit(1)
    ...
    result = subprocess.run([
        ...
        reference_project_file, "--with", silabs_board +
        config_args, "--force", "--generator-timeout=180", "-o", "makefile"
    ])
    ...
    result = subprocess.run(
        ["make", "all", "-C", output_dir, "-f", makefile_path, "-j13"])
    if result.returncode != 0:
