...
**This repo is intended to be used directly for bleeding edge development that closely tracks the CSA Matter codebase. If you are interested in a stable release, we recommend following our official documentation which can be found on [docs.silabs.com](https://docs.silabs.com/matter/2.9.1/matter-start/)**
...
```
More details on how to develop using SLC CLI can be found at [Creating Matter Applications using SLC CLI](https://docs.silabs.com/matter/2.9.1/matter-overview-guides/matter-slc-cli)
...
1. Run the slc/sl_project_upgrade.py script to upgrade the project using the checked-out Matter extension and SDKs.
1. For example:
   
`python3 slc/sl_project_upgrade.py MyNewApp/matter_thread_soc_lighting_app_freertos.slcp`
...
To load a custom Matter Extension in Studio, navigate to Settings -> SDKs -> Select the SDK to add extension to -> Add Extension. Navigate to the directory where the extension was staged, check the box next to `Silicon Labs Matter`, and click Finish.   
...
Note: Matter requires an up-to-date version of the ZAP tool. To make sure that your Studio installation has the most recent ZAP, follow these steps to trigger the Zap update: In the Simplicity Installer -> Package Manager, click Check for updates, then navigate to ZAP package. An Update button will be present if an update is available, click this. After a ZAP update, the existing Matter projects must be re-generated by clicking the three dots next to Project Details then "Force Generation" in the project window menu.
